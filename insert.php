    ...
    //check if exists in array or not
        if (in_array($submittedMonth, $bugInfo["season"])) {
          ...
          while ($i < count($bugInfo["season"]) -1) {
              ...
              if ($indexOfNext - $indexOfCurrent > 1) {
                  $allInfo["exp"] = array_search($bugInfo["season"][$i], $acnlMonths) - $yearlyIndex;
                  break;
              }
